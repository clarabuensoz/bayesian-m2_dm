...
model_3_bis<-function(){
  ...
  for(i in 1:n){
    y[i] ~ dpois(lambda[i]) #likelihood
    lambda[i]<-N*u[i]
    log(u[i]) <- a0[sites[i]] + log(A[i]) # + a1*x[i]
  }
  for(j in 1:nbsites){
...
We specify the initial values for 3 markov chains:
```{r}
init1 <- list(a0 = -4,mu.a0=1,sigma.a0=0.5)
init2 <- list(a0 = -3,mu.a0=-1,sigma.a0=0)
init3 <- list(a0 = -2,mu.a0=2,sigma.a0=-0.5)
init <- list(init1,init2,init3)
```

Parameters to monitor :
```{r}
params <- c("a0","mu.a0","sigma.a0")
```
