...
Comme les auteurs le font remarquer dans leur article, les données sont peu nombreuses.
...
### Model 5_bis - DO NOT CONVERGE - PROBLEM - TO DELETE
Y~ Poisson (T * u)
log (u) = a0 + Es + Ey + a1*P+ log(A)
Prior:
  a0~ N(0,sd=10)
  a1~ N(0,sd=10)
  Es~ N(mu.es,tau.es) #tau.es = 1/variance=sigma.es²
      ...
      tau.es <- 1 / (sigma.es * sigma.es)
      sigma.es~Unif(0,100)
  Ey~ N(mu.y,tau.y) #tau.es = 1/variance=sigma.ey²
      mu.y~N(0,sd=10)
...
mydata<-list(y=y,x=x,n=length(y),N=120,A=A,nbsites=nbsites,sites=sites,year=yearID,nbyear=nbyear)
```
...
##### Qualitativement par des graphiques:
```{r}
traceplot(model5_bis_fit,mfrow=c(1,2),varname=c("mu.a","sigma.a","a1","mu.y","sigma.y"),ask=FALSE)
````
...
## Part 2: more graph and test for model validation of Model2_bis and model4_bis (same as part assessing convergence for each model but with more beautiful graph)
### Chaine mixing et model validation::
Comparons 
Model 2_bis 
Modèle 4_bis 
...
Model 2_bis semble être le mieux fit au data selon les critères. 
Modèle 4_bis est assez proche également.

## Part 1/2: look for overdipersion:
Poisson regression: hypothèse de base est que variance=mean. Donc dispersion = 1.

Explore potential for overdispersion in data:
Source: https://www.flutterbys.com.au/stats/tut/tut10.6b.html
...
hist(data$shell)
```
Il semble y avoir beaucoup de 0 dans les données = potentiel overdispersion.
...
dat.tab/sum(dat.tab)
```
The value under TRUE is the proportion of zero value in the data.
The proportion of the value is the data do not correspond to the low proportion expected for a poisson distribution.

```{r}
#summary(model2_bis_fit)
library(BayesianTools)
Samples<-coda.samples(model2_bis_fit$model,variable.names = params, n.iter = 5000)
